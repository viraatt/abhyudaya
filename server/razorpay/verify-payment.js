...
// ── Firebase Admin (server-side, lazy) ───────────────────────
// firebase-admin v14 exposes getApps()/initializeApp/cert from
// "firebase-admin/app" — the legacy admin.apps accessor was removed.
// Initialization is lazy so a missing service-account env var never
// crashes the shared serverless function at cold start.
let _db = null;
function getDb() {
  if (!_db) {
    if (!getApps().length) {
